...
class SwapRequestService {
  ...
  // Lấy các tiết học của giáo viên để đổi tiết (chỉ lấy tiết scheduled)
  async getTeacherLessonsForSwap(
    teacherId,
    academicYear,
    startOfWeek,
    endOfWeek
    ...
    try {
      console.log(
        `🔍 Getting teacher lessons for swap - Teacher: ${teacherId}, Week: ${startOfWeek} to ${endOfWeek}`
      );
      ...
      // Tìm tất cả tiết học của giáo viên trong tuần đó (chỉ lấy tiết scheduled)
      const lessons = await Lesson.find({
        ...
        status: "scheduled", // Chỉ lấy tiết scheduled để đổi
      ...
      console.error(
        "❌ Error getting teacher lessons for swap:",
        error.message
      );
      throw new Error(
        `Failed to get teacher lessons for swap: ${error.message}`
      );
    }
  }

  // Lấy các tiết có giáo viên dạy để đổi tiết
  async getAvailableLessonsForSwap(
    classId,
    academicYear,
    startOfWeek,
    endOfWeek,
    ...
    try {
      console.log(
        `🔍 Getting available lessons for swap - Class: ${classId}, Subject: ${subjectId}`
      );
      ...
      // Tìm các tiết có giáo viên dạy trong lớp đó trong tuần (để đổi tiết)
      const availableLessons = await Lesson.find({
        ...
        type: { $in: ["regular", "makeup"] }, // Chỉ lấy tiết có giáo viên dạy
        status: "scheduled",
        teacher: { $exists: true, $ne: null }, // Phải có giáo viên dạy
      })
        .populate("class", "className gradeLevel")
        .populate("subject", "subjectName subjectCode")
      ...
      console.log(
        `📚 Found ${availableLessons.length} available lessons with teachers for swap`
      );
      ...
      console.error(
        "❌ Error getting available lessons for swap:",
        error.message
      );
      throw new Error(
        `Failed to get available lessons for swap: ${error.message}`
      );
    }
  }
  ...
  // Lấy thông tin chi tiết swap request
  async getSwapRequestDetails(requestId) {
    try {
      const lessonRequest = await LessonRequest.findById(requestId)
        .populate({
          path: "originalLesson",
          select:
            "lessonId scheduledDate timeSlot topic status type teacher subject",
          populate: [
            {
              path: "timeSlot",
              select: "period name startTime endTime",
      ...
      console.error("❌ Error getting swap request details:", error.message);
      throw new Error(`Failed to get swap request details: ${error.message}`);
    }
  }
