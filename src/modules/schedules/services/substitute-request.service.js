...
class SubstituteRequestService {
  ...
  async createSubstituteRequest(lessonId, requestingTeacherId, candidateTeacherIds, reason) {
    try {
      ...
      if (lesson.status !== 'scheduled') {
        throw new Error('Can only create substitute requests for scheduled lessons');
      }
      ...
      if (candidateTeachers.length !== candidateTeacherIds.length) {
        throw new Error('Some candidate teachers not found or not valid teachers');
      }
  ...
  async getSubstituteRequestById(requestId) {
    const request = await SubstituteRequest.findById(requestId)
      .populate({
        path: 'lesson',
        populate: [
  ...
  async getTeacherRequests(teacherId, status = null) {
    return await SubstituteRequest.getTeacherRequests(teacherId, status);
  }
  ...
  async approveRequest(requestId, teacherId) {
    try {
      ...
      // Check if teacher is in candidate list with proper ObjectId handling
      ...
      if (!isCandidate) {
        console.error('❌ Teacher not authorized. Teacher ID:', teacherIdStr);
        console.error('❌ Candidates:', request.candidateTeachers.map(c => ({
          id: c.teacher._id ? c.teacher._id.toString() : c.teacher.toString(),
          name: c.teacher.name
        })));
  ...
  async getAvailableTeachers(lessonId) {
    return await SubstituteRequest.findAvailableTeachers(lessonId);
  }
  ...
  async cancelOtherTeacherRequests(teacherId, excludeRequestId) {
    try {
      ...
      const otherRequests = await SubstituteRequest.find({
      ...
      console.log(`Cancelled/updated ${otherRequests.length} other requests for teacher ${teacherId}`);
    } catch (error) {
  ...
  async sendRequestEmails(requestId) {
    try {
      ...
      const candidateEmails = request.candidateTeachers.map(c => c.teacher.email);
      
      const lessonDate = new Date(request.lesson.scheduledDate).toLocaleDateString('vi-VN');
      ...
      const managerEmailHtml = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="background-color: #f8f9fa; padding: 20px; border-radius: 5px; margin: 20px 0;">
            ...
            <p><strong>Thời gian:</strong> ${lessonTime}</p>
            <p><strong>Lý do:</strong> ${request.reason}</p>
            <p><strong>Giáo viên được đề xuất:</strong> ${request.candidateTeachers.map(c => c.teacher.name).join(', ')}</p>
          </div>
          <hr style="margin: 30px 0;">
          <p style="color: #7f8c8d; font-size: 12px;">Email này được gửi tự động từ hệ thống EcoSchool.</p>
        </div>
      ...
      await Promise.all([
        ...candidateEmails.map(email => 
          emailService.sendEmail(email, 'Yêu cầu dạy bù - EcoSchool', candidateEmailHtml)
        ),
        ...managerEmails.map(email => 
          emailService.sendEmail(email, 'Thông báo yêu cầu dạy bù - EcoSchool', managerEmailHtml)
        )
      ]);

      // Record sent emails
      await SubstituteRequest.findByIdAndUpdate(requestId, {
        $push: {
          emailsSent: {
            type: 'request',
  ...
  async sendApprovalEmails(requestId) {
    try {
      ...
      const otherCandidates = request.candidateTeachers.filter(
        c => c.teacher._id.toString() !== request.approvedTeacher._id.toString()
      );
      ...
      await Promise.all(
        allRecipients.map(email => 
          emailService.sendEmail(email, 'Yêu cầu dạy bù đã được chấp nhận - EcoSchool', 
            email === request.requestingTeacher.email ? requestingTeacherEmailHtml : notificationEmailHtml)
        )
      );
  ...
  async sendRejectionEmails(requestId, rejectingTeacherId, reason) {
    try {
      const request = await this.getSubstituteRequestById(requestId);
      const rejectingTeacher = await User.findById(rejectingTeacherId).select('name');
      ...
      const rejectionEmailHtml = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <p>Xin chào ${request.requestingTeacher.name},</p>
          <p>Giáo viên ${rejectingTeacher.name} đã từ chối yêu cầu dạy bù của bạn:</p>
    ...
    try {
      ...
      await Promise.all(
        studentEmails.map(email => 
          emailService.sendEmail(email, 'Thông báo thay đổi giáo viên - EcoSchool', studentEmailHtml)
        )
      );
      ...
      console.log(`Sent teacher change notification to ${studentEmails.length} students`);
  ...
  async getAllRequests(status = null, page = 1, limit = 20) {
    ...
    const requests = await SubstituteRequest.find(query)
      .populate({
        path: 'lesson',
        populate: [
          { path: 'class', select: 'className' },
    ...
    const total = await SubstituteRequest.countDocuments(query);
