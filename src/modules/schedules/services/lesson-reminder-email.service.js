...
class LessonReminderEmailService {
  ...
  createReminderEmailTemplate(reminderData, studentName) {
    const { lesson, class: classInfo, subject, testType, title, content, expectedTestDate, priority, chapters, references } = reminderData;
            ...
            <div style="padding: 20px;">
              <div style="display: grid; gap: 15px;">
                <div style="display: flex; align-items: center;">
                  ...
                  <span style="color: #007bff; font-weight: 500;">${subject.name} (${subject.code})</span>
                </div>
                <div style="display: flex; align-items: center;">
                  <span style="font-weight: 600; color: #495057; width: 120px; display: inline-block;">🏫 Lớp:</span>
                  <span style="color: #28a745; font-weight: 500;">${classInfo.className}</span>
                </div>
                <div style="display: flex; align-items: center;">
                  <span style="font-weight: 600; color: #495057; width: 120px; display: inline-block;">📝 Loại:</span>
                  <span style="color: #6f42c1; font-weight: 500;">${testTypeNames[testType] || testType}</span>
                </div>
                <div style="display: flex; align-items: center;">
                  <span style="font-weight: 600; color: #495057; width: 120px; display: inline-block;">⚡ Mức độ:</span>
                  <span style="background: ${priorityColors[priority]}; color: white; padding: 4px 12px; border-radius: 15px; font-size: 12px; font-weight: 600; text-transform: uppercase;">
          ...
          <div style="margin-bottom: 25px;">
            ...
            ${chapters.map(chapter => `
              <div style="background: white; border: 1px solid #dee2e6; border-radius: 8px; padding: 15px; margin-bottom: 15px;">
                <h5 style="color: #007bff; margin: 0 0 10px; font-size: 15px;">${chapter.chapterName}</h5>
                ${chapter.topics && chapter.topics.length > 0 ? `
                  <ul style="margin: 0; padding-left: 20px; color: #666;">
                    ${chapter.topics.map(topic => `<li style="margin-bottom: 5px;">${topic}</li>`).join('')}
                  </ul>
                ` : ''}
              </div>
            `).join('')}
          ...
          <div style="margin-bottom: 25px;">
            ...
            ${references.map(ref => `
              <div style="background: #e3f2fd; border-left: 4px solid #2196f3; padding: 15px; margin-bottom: 10px; border-radius: 0 8px 8px 0;">
                <h6 style="margin: 0 0 8px; color: #1976d2; font-size: 14px;">${ref.title}</h6>
                ${ref.description ? `<p style="margin: 0 0 8px; color: #666; font-size: 13px;">${ref.description}</p>` : ''}
                ${ref.url ? `<a href="${ref.url}" style="color: #1976d2; text-decoration: none; font-size: 13px;">🔗 Xem tài liệu</a>` : ''}
              </div>
            `).join('')}
          </div>
          ` : ''}
  ...
  async sendReminderToStudents(reminderData, students) {
    try {
      console.log(`📧 Sending reminder emails to ${students.length} students...`);
      
      const emailPromises = students.map(async (student) => {
        try {
          const subject = `🔔 Thông báo kiểm tra: ${reminderData.subject.name} - Lớp ${reminderData.class.className}`;
          const htmlContent = this.createReminderEmailTemplate(reminderData, student.name);
          
          const result = await emailService.sendEmail(student.email, subject, htmlContent);
          
          console.log(`✅ Email sent to ${student.name} (${student.email}): ${result.messageId}`);
          ...
          console.error(`❌ Failed to send email to ${student.name} (${student.email}):`, error.message);
  ...
  async sendReminderToStudent(reminderData, student) {
    try {
      ...
      console.log(`✅ Reminder email sent to ${student.name} (${student.email}): ${result.messageId}`);
      ...
      console.error(`❌ Failed to send reminder email to ${student.name}:`, error);
      throw error;
    }
  }
  ...
  async sendTestReminderEmail(testEmail, reminderData) {
    try {
      ...
      const htmlContent = this.createReminderEmailTemplate(reminderData, 'Test Student');
      
      const result = await emailService.sendEmail(testEmail, subject, htmlContent);
      
      console.log(`✅ Test reminder email sent to ${testEmail}: ${result.messageId}`);
  ...
  createCancelReminderEmailTemplate(reminderData, studentName) {
    const { lesson, class: classInfo, subject, testType, title, content, expectedTestDate } = reminderData;
    ...
    try {
      ...
      const emailPromises = students.map(async (student) => {
        try {
          ...
          const htmlContent = this.createCancelReminderEmailTemplate(reminderData, student.name);
          
          const result = await emailService.sendEmail(student.email, subject, htmlContent);
          
          console.log(`✅ Cancellation email sent to ${student.name} (${student.email}): ${result.messageId}`);
          ...
          console.error(`❌ Failed to send cancellation email to ${student.name} (${student.email}):`, error.message);
      ...
      console.log(`📊 Cancellation email summary: ${successCount} successful, ${failureCount} failed out of ${students.length} total`);
