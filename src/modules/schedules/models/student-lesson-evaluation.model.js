...
const studentLessonEvaluationSchema = new mongoose.Schema({
  ...
  evaluation: {
    ...
    teacherInteraction: {
      ...
      validate: {
        validator: Number.isInteger,
        message: 'Teacher interaction rating must be an integer between 1 and 5'
      }
    },
...
studentLessonEvaluationSchema.index({ student: 1, lesson: 1 }, { unique: true }); // Mỗi học sinh chỉ đánh giá 1 lần cho 1 tiết
...
studentLessonEvaluationSchema.pre('save', function(next) {
  ...
  const { teachingClarity, teachingSupport, teacherInteraction } = this.evaluation;
  this.evaluation.overallRating = Math.round((teachingClarity + teachingSupport + teacherInteraction) / 3 * 10) / 10;
...
studentLessonEvaluationSchema.pre('save', async function(next) {
  try {
    ...
    // 4. Kiểm tra học sinh có trong danh sách điểm danh của tiết học (đã tham gia tiết học)
    const wasPresent = !lesson.attendance.absentStudents.some(
      absent => absent.student.toString() === this.student.toString()
    );
    
    if (!wasPresent) {
      throw new Error('Student was not present in this lesson and cannot evaluate it');
    }
...
studentLessonEvaluationSchema.statics.getStudentEvaluations = function(studentId, options = {}) {
...
studentLessonEvaluationSchema.statics.getLessonEvaluations = function(lessonId) {
...
studentLessonEvaluationSchema.statics.getTeacherEvaluationStats = async function(teacherId, options = {}) {
  const matchQuery = { teacher: teacherId };
  
  if (options.startDate) matchQuery.evaluatedAt = { $gte: options.startDate };
  if (options.endDate) {
    matchQuery.evaluatedAt = { ...matchQuery.evaluatedAt, $lte: options.endDate };
  }
  ...
  return {
    ...
    completedWellRate: Math.round((result.completedWellCount / result.totalEvaluations) * 100)
  };
};

// Static method để kiểm tra học sinh có thể đánh giá tiết học không
studentLessonEvaluationSchema.statics.canStudentEvaluateLesson = async function(studentId, lessonId) {
  try {
    const User = mongoose.model('User');
    const Lesson = mongoose.model('Lesson');
    ...
    if (student.class_id._id.toString() !== lesson.class.toString()) {
      return { canEvaluate: false, reason: 'Student does not belong to this class' };
    }
    
    // Kiểm tra học sinh có tham gia tiết học (không vắng mặt)
    const wasAbsent = lesson.attendance.absentStudents.some(
      absent => absent.student.toString() === studentId
    );
    
    if (wasAbsent) {
      return { canEvaluate: false, reason: 'Student was absent from this lesson' };
    }
    ...
    if (existingEvaluation) {
      return { canEvaluate: false, reason: 'Student has already evaluated this lesson' };
    }
...
studentLessonEvaluationSchema.methods.updateEvaluation = function(newEvaluation, newComments, newSelfAssessment) {
  if (newEvaluation) {
  ...
  if (newSelfAssessment) {
    this.studentSelfAssessment = { ...this.studentSelfAssessment, ...newSelfAssessment };
  }
...
const StudentLessonEvaluation = mongoose.model('StudentLessonEvaluation', studentLessonEvaluationSchema);
