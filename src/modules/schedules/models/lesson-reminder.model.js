...
const lessonReminderSchema = new mongoose.Schema({
  ...
  testType: {
    type: String,
    enum: ['kiemtra15', 'kiemtra1tiet', 'kiemtrathuchanh', 'kiemtramieng', 'baitap', 'other'],
    required: true,
    default: 'kiemtra15'
  },
...
lessonReminderSchema.pre('save', async function(next) {
  try {
    ...
    if (lesson.teacher.toString() !== this.teacher.toString()) {
      throw new Error('Teacher can only create reminders for their own lessons');
    }
...
// Static methods
lessonReminderSchema.statics.getTeacherReminders = function(teacherId, options = {}) {
  ...
  if (options.endDate) {
    query.expectedTestDate = { ...query.expectedTestDate, $lte: options.endDate };
  }
...
lessonReminderSchema.statics.getUpcomingReminders = function(teacherId, days = 7) {
...
lessonReminderSchema.statics.getReminderStats = async function(teacherId, options = {}) {
