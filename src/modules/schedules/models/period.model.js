...
periodSchema.pre('save', function(next) {
  ...
  if (!this.periodId && this.schedule && this.weekNumber && this.dayOfWeek && this.periodNumber) {
    ...
    console.log(`🆔 Generated periodId: ${this.periodId} for Schedule: ${this.schedule}`);
  }
...
periodSchema.index({ schedule: 1, weekNumber: 1, dayOfWeek: 1, periodNumber: 1 });
...
periodSchema.index({ 'location.weekNumber': 1, 'location.dayOfWeek': 1, 'location.periodNumber': 1 });
...
periodSchema.statics.checkTeacherConflict = async function(teacherId, date, periodNumber, excludePeriodId = null) {
  const query = {
    teacher: teacherId,
    date: date,
...
periodSchema.statics.getTeacherPeriods = function(teacherId, startDate, endDate) {
  return this.find({
    teacher: teacherId,
...
periodSchema.statics.bulkCreatePeriods = async function(periodsData) {
  try {
    ...
    const validatedData = periodsData.map((period, index) => {
      ...
      if (!period.periodId && period.schedule && period.weekNumber && period.dayOfWeek && period.periodNumber) {
      ...
      if (!period.periodId) {
        throw new Error(`Period ${index + 1}: Cannot generate periodId - missing required fields`);
      }
      
      if (!period.class || !period.schedule || !period.createdBy) {
        throw new Error(`Period ${index + 1}: Missing required fields (class, schedule, or createdBy)`);
      }
    ...
    if (error.code === 11000) {
      ...
      console.log(`⚠️ Partial success: ${successful.length} periods created with some duplicates`);
      return {
        success: false,
...
periodSchema.methods.convertToMakeup = function(teacherId, subjectId, makeupInfo, updatedBy) {
  if (this.periodType !== 'empty') {
...
periodSchema.methods.convertToExtracurricular = function(teacherId, extracurricularInfo, updatedBy) {
  if (this.periodType !== 'empty') {
