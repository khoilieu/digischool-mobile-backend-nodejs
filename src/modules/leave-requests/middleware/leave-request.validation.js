...
// Validation for creating leave requests
const validateCreateLeaveRequest = [
  body('lessonIds')
...
const validateBatchProcess = [
  ...
  body('requests.*.comment')
    ...
    .withMessage('Comment cannot exceed 300 characters')
    .custom((comment, { req }) => {
      const requestIndex = req.body.requests.findIndex(r => r.comment === comment);
      const request = req.body.requests[requestIndex];
      
      if (request && request.action === 'reject' && (!comment || !comment.trim())) {
...
module.exports = {
  validateCreateLeaveRequest: [...validateCreateLeaveRequest, handleValidationErrors],
  ...
  validateAvailableLessons: [...validateAvailableLessons, handleValidationErrors],
